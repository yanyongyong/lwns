...
<table id="dg" title="权限信息" style="width:100%;height: 520px;" class="easyui-datagrid"
    ...
    <tr>
        ...
        <th data-options="field:'memberPhone',width:120">电话号码</th>
        <th data-options="field:'memberAddress',width:190">地址</th>
        <th data-options="field:'createdStore',width:160,">办卡地点</th>
        <th data-options="field:'memberIntegral',width:100,">积分</th>
...
<script type="text/javascript">
    ...
    function editMember() {
        ...
        if (row) {
            $('#editDlg').dialog('open').dialog('setTitle', '会员充值');
            $('#deit').form('load', row);
            url = '/member/MemberRecharge?memberId=' + row.memberId;
...
<div id="editDlg" class="easyui-dialog"
    ...
    <form id="deit" method="post" novalidate>
        ...
        <div class="fitem">
            <label> 余额: </label> <input name="memberBalance" class="easyui-validatebox textbox">
        </div>

        <div class="fitem">
            <label> 充值金额: </label> <input name="recharge" class="easyui-validatebox textbox">
        </div>
...
<div style="background:red; width:200px; height: 25px; line-height: 25px; text-align: center; position: absolute; left: 50%; top:0; margin-left: -100px;">
    ${sessionScope.memberResult}
</div>
